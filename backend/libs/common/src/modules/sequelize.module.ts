...
export interface DatabaseConfig {
  ...
  APP_DATABASE_IDLE_TRANSACTION_SESSION_TIMEOUT: string;
}

interface ExtendedDialectOptions {
...
/**
 * Sequelize client required constructor to be used in RepositoryParam decorator.
 */
export interface DatabaseRepositoryDefault {
      ...
      providers: [
        {
          ...
          /**
           * Create a unique sequelize instance.
          ...
          useFactory: async (
            ...
            if (!DatabaseModule.sequelize) {
              ...
              logger = logger?.child({ context: 'SequelizeModule' });
              ...
              const poolIdle = Number(
                configService.get<string>('APP_DATABASE_POOL_IDLE') ?? 10000,
              );
              const idleInTransactionSessionTimeout = Number(
                configService.get<string>(
                  'APP_DATABASE_IDLE_TRANSACTION_SESSION_TIMEOUT',
                ) ?? 10000,
              );
              ...
              const options: SequelizeOptions = {
                ...
                pool: { max, acquire, idle: poolIdle },
              };

              // Should use idle in transaction timeout.
              if (idleInTransactionSessionTimeout > 0) {
                (
                  options.dialectOptions as ExtendedDialectOptions
